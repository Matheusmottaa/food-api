...
public class RegisterCityService {
	...
	public City save(City city) throws ResourceNotFoundException {
		...
		if (state == null) {
			throw new ResourceNotFoundException(String.format("There's no state register with id %d\n", stateId));
		}
		city.setState(state);
	...
	public void remove(Long cityId) throws ResourceNotFoundException {
		if (!cityRepository.existsById(cityId)) {
			throw new ResourceNotFoundException(String.format("There's no city register with id %d\n", cityId));
		}
